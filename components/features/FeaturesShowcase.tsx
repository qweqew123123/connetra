...
export default function FeaturesShowcase() {
  ...
  const ActiveVisualComponent = visualComponents[activeFeature.visual];

  return (
    <>
      {/* Desktop Version: Sticky Scroll Showcase */}
      <section className="fp-showcase-track fp-desktop-only" ref={trackRef} aria-label="Feature Showcase Desktop">
            ...
            <div className="fp-section-grid fp-showcase-grid">
              ...
              <div className="fp-section-visual fp-showcase-visual" key={`visual-${activeFeature.number}`}>
                {activeFeature.video ? (
                  ...
                  <ActiveVisualComponent key={activeFeature.number} entered={true} />
                )}
              </div>
            </div>
          </div>
      ...
      {/* Mobile Version: Simple Stacked 1-by-1 Layout (No Sticky Scroll) */}
      <section className="fp-mobile-stacked fp-mobile-only" aria-label="Feature Showcase Mobile">
        <div className="fp-container">
          <div className="fp-mobile-feature-list">
            {featuresPage.map((feat) => {
              const VisualComp = visualComponents[feat.visual];
              return (
                <div className="fp-mobile-feature-card" key={feat.number}>
                  <div className="fp-mobile-feature-copy">
                    <div className="fp-eyebrow-badge">
                      <span className="dot" />
                    ...
                    <a className="fp-text-cta" href="/integrations">
                      {feat.cta} <ArrowRight size={16} aria-hidden="true" />
                    </a>
                  </div>
                  <div className="fp-mobile-feature-visual">
                    {feat.video ? (
                      <FeatureVideo
                        src={feat.video}
                        label={feat.title}
